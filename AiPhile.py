...
# draw text with background having outlines 
def textBGoutline(img, text, position=(40,30), fonts=cv.FONT_HERSHEY_SIMPLEX ,scaling=1, text_color=(0,255,0), thickness=1, bg_color=(BLACK)):
    img_h, img_w = img.shape[:2]
    x, y = position
    (w, h ), p= cv.getTextSize(text, fonts, scaling, thickness)
    # print(w, h)
...
# draw shape with transparency, from 0. to 1
